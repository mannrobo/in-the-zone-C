...
#pragma config(Motor,  port7,           leftCone,      tmotorVex393_MC29, openLoop, reversed)
...
void longAuto() {
		//score the closest mobile goal in the 5 point zone
		autoRightSide5Points();
		//back up
		autoBackward(1000);
		//turn around
		autoLeft(1800);
		//go toward the farther mobile goal and pick it up
		autoForward(3000);
		while(SensorValue(LSbumper) == 0) {
				autoForward(100);
				wait1Msec(10);
		...
		stopDrive();
		//go forward and put the mobile goal in the opposite 5 point zone from the former
		autoForward(2000);
}
